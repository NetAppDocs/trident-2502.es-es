...
| `luksEncryption` | Active el cifrado LUKS. Consulte link:../trident-reco/security-luks.html["Usar la configuración de clave unificada de Linux (LUKS)"]. El cifrado LUKS no es compatible con NVMe/TCP. | Ajuste en `false` para ASA R2. 
